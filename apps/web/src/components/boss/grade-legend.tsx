...
import { GRADES, GRADE_LABEL, GRADE_STYLE, gradeDescription } from "@/lib/boss";

interface GradeLegendProps {
  /** 목록 응답의 젠 간격 — 보스마다 다를 수 있어 설명 문구에 그대로 쓴다 */
  spawn: { minHours: number; maxHours: number };
}

/** 등급 기준표를 info 아이콘 옆 레이어로 띄운다 */
export function GradeLegend({ spawn }: GradeLegendProps) {
      ...
      {open && (
        ...
        >
          ...
          <p className="mt-0.5 text-caption text-grey-500">
            처치 후 {spawn.minHours}시간이 지나면 출현해요.
          </p>
          ...
          <ul className="mt-3 flex flex-col gap-2">
            {GRADES.map((grade) => (
              <li key={grade} className="flex items-center gap-2.5">
                ...
                <span className="text-caption text-grey-500">{gradeDescription(grade, spawn)}</span>
              </li>
            ))}
          </ul>
        </div>
