...
- 카드는 **세 줄**이다 — `채널 번호 + 처치자 아이디` / `등급 - 시간` / `출현 안함 확인 시각`.
  셋째 줄은 확인 기록이 있을 때만 그려지지만 카드 최소 높이(`min-h-15`, 60px)가 세 줄 기준이라
  줄이 없어도 칸 크기는 흔들리지 않는다. 한 줄에 놓이는 개수는 반응형
  ...
  이므로 글자 크기를 키울 때는 거기서 재 볼 것. 여기에 배지·버튼을 더 넣지 말 것
- **미확인 카드의 시간은 늘 `--:--`** — 기록이 없든 출현 후 6시간이 넘어 못 믿게 됐든 마찬가지다.
  숫자를 남겨두면 아직 쓸 수 있는 타이머처럼 읽힌다. 실제 경과 시간은 모달에서 본다
- **확인 시각은 경과가 아니라 시각(`13:20`)으로 적는다** — "언제 다녀갔나" 가 궁금한 정보이고,
  덕분에 그 줄은 매분 다시 그릴 필요가 없다
...
- **"확인" 기록은 처치와 다른 동작이다** — 가봤는데 아직 보스가 안 나왔을 때 다녀온 시각을
  ...
  다시 돌리면 안 된다
- **처치를 기록해도 확인 기록은 지우지 않는다** — 처치 후에도 "마지막으로 언제 가봤는지" 를
  카드에 함께 보여준다. `CLEAR_CHECK` 는 기록을 통째로 비우는 동작(타이머 초기화,
  `lastKilledAt: null` 수정)에만 쓸 것
- **모달의 기록 버튼은 두 개**(`처치함` / `출현 안함`)이고 **둘 다 등급과 무관하게 늘 보인다** —
  확인 버튼에 등급 조건을 걸지 말 것. 조건을 걸면 방금 처치한 채널에 이전 헛걸음을 적어둘
  방법이 사라진다. "시간 직접 입력" 체크박스는 **두 버튼에 똑같이** 적용된다 —
  확인 전용 입력칸을 따로 두지 말 것
- 처치자 이름은 채널 번호와 **같은 줄**에 붙인다 (줄 수를 늘리지 않기 위해)
