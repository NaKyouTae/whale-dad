...
- 모달에서 **처치 시각을 수기로 입력**할 수 있다 (놓친 처치를 나중에 적는 용도).
  ...
  입력 도중에 값이 바뀌어 버린다
- **"확인" 기록은 처치와 다른 동작이다** — 출현 시각이 지났는데 가봤더니 보스가 없을 때
  다녀온 시각을 남긴다(`lastCheckedAt`). **타이머를 절대 건드리지 말 것** — 확인은 처치가
  아니므로 젠 주기를 다시 돌리면 안 된다. 반대로 처치를 새로 기록하거나 타이머를 초기화하면
  확인 기록은 함께 지운다(`CLEAR_CHECK`) — 지난 주기의 확인은 의미가 없으므로.
  모달의 확인 칸은 `grade === "SPAWNED"` 일 때만 보인다 (출현 전이면 없는 게 당연하므로)
