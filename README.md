...
| `pnpm db:seed` | 채널 1~231 시드 (범위 밖 채널은 숨김 처리) |
| `pnpm db:studio` | Prisma Studio |

## 여두목 보스 타이머

메이플플래닛 여두목 보스는 **처치 후 3시간이 지나면 출현**한다. 채널을 돌며 잡을 때마다 카드를
눌러 그 채널의 타이머를 돌리는 화면이다. (`/boss`)
...
- **채널 위치는 채널 번호 순으로 고정**이다. 한 줄에 10개씩 놓이고, 카드는 채널 번호 + 타이머
  두 줄뿐이다. 등급은 라벨이 아니라 **카드 색**으로 구분한다
- 타이머는 출현 전이면 **출현까지 남은 시간**, 출현 뒤면 **지난 시간(`+HH:MM:SS`)** 을 보여준다
...
| 엔드포인트 | 설명 |
| --- | --- |
| `GET /api/boss-channels` | 채널 목록 + 출현 시각 + 서버 시각 |
| `POST /api/boss-channels/:ch/kill` | 처치 기록 (타이머 시작) · 로그인 필요 |
| `DELETE /api/boss-channels/:ch/kill` | 타이머 초기화 · 로그인 필요 |
| `PATCH /api/boss-channels/:ch` | 처치 시각 / 메모 / 노출 여부 수정 · 로그인 필요 |
| `POST /api/boss-channels` · `DELETE /api/boss-channels/:ch` | 채널 추가 / 삭제 · 로그인 필요 |
| `POST /api/boss-channels/sync` | 범위 일괄 동기화 (예: `{"from":1,"to":231}`) · 로그인 필요 |
...
2. 채널 테이블이 **비어 있을 때만** 기본 범위(1~231) 생성
   — 이미 채널이 있으면(범위를 조정했을 수 있으므로) 건드리지 않는다
