...
메이플플래닛 보스는 처치하고 일정 시간이 지나면 다시 출현한다. 채널을 돌며 잡을 때마다 카드를
눌러 그 채널의 타이머를 돌리는 화면이다.

보스는 좌측 사이드바에서 고르고, 화면은 보스마다 완전히 같다. 채널 목록·타이머는 보스별로
따로 관리되고, **재출현 간격도 보스마다 다르다**.

| 보스 | 경로 | 재출현 |
| --- | --- | --- |
| 여두목 보스 | `/boss/yeodumok` | 처치 후 3~5시간 |
| 천구 보스 | `/boss/cheongu` | 처치 후 6~8시간 |

보스를 하나 더 붙이려면 `packages/shared` 의 `BOSS_DEFINITIONS` 에 한 줄 추가하고
Prisma `BossType` enum 에 같은 값을 더하면 된다 — 메뉴·라우트·API·부팅 시드가 모두 따라온다.

등급은 서버가 내려준 `earliestSpawnAt`(처치 + 보스별 최소 젠 시간)까지 남은 시간으로 클라이언트가
**매초** 다시 계산한다. 경계는 출현 직전을 기준으로 잡아 **젠 간격이 달라도 보스 공통**이다.

| 등급 | 기준 | 색 |
| --- | --- | --- |
| 안전 | 출현까지 1시간 넘게 남음 | 초록 |
| 주의 | 출현 1시간 전 | 노랑 |
| 위험 | 출현 10분 전 | 빨강 |
| 출현 | 출현 시각이 지남 | 파랑 (유일하게 꽉 채운 색) |
| 기록 없음 | 처치 기록 없음 | 회색 |
...
- 카드의 타이머는 **처치 후 흐른 시간**이다. `00:00` 에서 시작해 분 단위로 올라가고,
  그 보스의 최소 젠 시간(여두목 `03:00` · 천구 `06:00`)을 넘기면 출현 상태가 된다.
  초는 카드에 띄우지 않는다 — 232개가 매초 바뀌면 읽히지 않는다 (정확한 초는 모달에서 본다)
...
- 출현 시각이 지났는데 **가봤더니 보스가 없을 때**는 모달의 확인 칸에 다녀온 시각을 적어
  남길 수 있다. 같은 채널을 여럿이 헛걸음하지 않게 하는 용도라 **타이머는 그대로 둔다**.
  처치를 새로 기록하면 확인 기록은 지워진다
...
| `POST /api/bosses/:boss/channels/:ch/check` | 확인 기록 (가봤는데 없었음, 타이머 유지) · 로그인 필요 |
